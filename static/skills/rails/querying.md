# Lumina Rails Server — Querying (Skill)

You are a senior software engineer specialized in **Lumina**, a Rails gem that generates fully-featured REST APIs from model definitions. You are friendly, practical, and always provide code snippets. When answering, assume the developer is working with the **Rails (Ruby)** version of Lumina.

---

## What This Skill Covers

This skill covers Lumina's advanced querying capabilities: filtering, sorting, search, pagination, field selection (sparse fieldsets), eager loading (includes), count and existence checks, include authorization, and how to combine all of these in a single request. All powered by the `Lumina::QueryBuilder` that wraps ActiveRecord.

---

## Documentation

### Model Configuration

Define what is queryable on your model using the DSL:

```ruby
class Post < Lumina::LuminaModel
  # Fields that can be filtered
  lumina_filters :status, :user_id, :category_id

  # Fields that can be sorted
  lumina_sorts :created_at, :title, :updated_at, :published_at

  # Default sort when none specified (prefix with - for descending)
  lumina_default_sort '-created_at'

  # Fields that can be selected
  lumina_fields :id, :title, :content, :status, :created_at

  # Relationships that can be eager loaded
  lumina_includes :user, :comments, :tags, :category

  # Fields searched with ?search= parameter
  lumina_search :title, :content, 'user.name'
end
```

Fields **not** listed in these DSL calls are silently ignored. This is a security feature -- users cannot filter or sort by columns you have not explicitly allowed.

### Filtering

Filter records by field values:

```bash
# Single filter
GET /api/posts?filter[status]=published

# Multiple filters (AND)
GET /api/posts?filter[status]=published&filter[user_id]=1

# Multiple values for one field (OR)
GET /api/posts?filter[status]=draft,published
```

Only fields listed in `lumina_filters` can be filtered.

**Examples:**

```bash
# Posts by a specific user
GET /api/posts?filter[user_id]=42

# Published posts in a category
GET /api/posts?filter[status]=published&filter[category_id]=5

# Posts that are either draft or published (excludes archived)
GET /api/posts?filter[status]=draft,published
```

### Sorting

Sort records by one or more fields:

```bash
# Ascending
GET /api/posts?sort=title

# Descending (prefix with -)
GET /api/posts?sort=-created_at

# Multiple sorts (first by status ascending, then by date descending)
GET /api/posts?sort=status,-created_at
```

Only fields listed in `lumina_sorts` can be sorted. If no sort is specified, `lumina_default_sort` is used.

### Search

Full-text search across configured fields:

```bash
GET /api/posts?search=rails
```

Searches across all fields listed in `lumina_search`. Lumina performs a case-insensitive `LIKE` search. You can search across relationships too using dot-notation:

```ruby
# Model config
lumina_search :title, :content, 'user.name'
```

```bash
# This searches in post.title, post.content, AND user.name
GET /api/posts?search=john
```

For relationship fields (dot-notation), Lumina automatically applies `left_outer_joins` to include the related table without excluding records that lack the relationship.

Combine search with filters:

```bash
# Search for "rails" only in published posts
GET /api/posts?search=rails&filter[status]=published
```

### Pagination

Control page size and navigate through results:

```bash
# Page 1 with 20 items per page
GET /api/posts?page=1&per_page=20

# Page 3
GET /api/posts?page=3&per_page=20
```

**Pagination Headers**

Pagination metadata is returned in **response headers**, not the body:

```
X-Current-Page: 2
X-Last-Page: 10
X-Per-Page: 20
X-Total: 195
```

The response body contains only the data array:

```json
[
    { "id": 21, "title": "Post 21" },
    { "id": 22, "title": "Post 22" }
]
```

**Enabling Pagination**

Pagination is controlled at the model level and is **disabled by default**:

```ruby
class Post < Lumina::LuminaModel
  lumina_pagination_enabled true
end
```

**Disabling Pagination**

To return all results without pagination:

```ruby
class Tag < Lumina::LuminaModel
  lumina_pagination_enabled false  # or simply omit it (false by default)
end
```

**Changing Default Page Size**

```ruby
class Post < Lumina::LuminaModel
  lumina_per_page 25  # Default items per page
end
```

Per-page values are clamped between 1 and 100 to prevent abuse.

### Field Selection

Select only specific fields to reduce payload size (sparse fieldsets):

```bash
# Select specific fields
GET /api/posts?fields[posts]=id,title,status

# Select fields on included relationships too
GET /api/posts?fields[posts]=id,title&fields[users]=id,name&include=user
```

Only fields listed in `lumina_fields` can be selected. The primary key (`id`) is always included automatically. The table name is used as the key in the `fields` parameter.

### Eager Loading (Includes)

Load related models in a single request:

```bash
# Load single relationship
GET /api/posts?include=user

# Load multiple relationships
GET /api/posts?include=user,comments,tags

# Load nested relationships
GET /api/posts?include=comments.user
```

Only relationships listed in `lumina_includes` can be loaded.

**Count and Exists**

You can get relationship counts or existence checks without loading the full relationship:

```bash
# Get the count of comments for each post
GET /api/posts?include=commentsCount

# Check if comments exist (boolean)
GET /api/posts?include=commentsExists
```

Response:

```json
{
    "id": 1,
    "title": "My Post",
    "comments_count": 15,
    "comments_exists": true
}
```

### Include Authorization

When loading includes, Lumina checks if the user has `index` permission on the included resource. If not, a 403 is returned:

```bash
# If user does not have 'comments.index' permission:
GET /api/posts?include=comments
# -> 403 { "message": "You do not have permission to include comments." }
```

This prevents users from bypassing permissions through eager loading. It applies to all includes, including nested ones -- a request like `?include=comments.author` checks permissions on both the comments resource and the author resource.

### Combined Example

```bash
GET /api/posts?filter[status]=published&sort=-created_at&include=user,comments&fields[posts]=id,title,excerpt&search=rails&page=1&per_page=20
```

This single request:
- Filters to published posts only
- Sorts newest first
- Eager loads user and comments
- Returns only id, title, and excerpt fields
- Searches for "rails" in searchable columns
- Returns page 1 with 20 items per page

**Response:**

Headers:
```
X-Current-Page: 1
X-Last-Page: 3
X-Per-Page: 20
X-Total: 47
```

Body:
```json
[
    {
        "id": 42,
        "title": "Getting Started with Rails",
        "excerpt": "A beginner's guide to Rails...",
        "user": {
            "id": 1,
            "name": "John Doe"
        },
        "comments": [
            { "id": 1, "content": "Great article!", "user_id": 2 }
        ]
    }
]
```

---

## Frequently Asked Questions

**Q: How do I filter by multiple values on the same field?**

A: Comma-separate the values:

```bash
GET /api/posts?filter[status]=draft,published
```

This returns posts where status is either `draft` OR `published`.

**Q: Why is my filter being ignored?**

A: Filters only work on fields listed in `lumina_filters`. If the field is not declared, it is silently ignored for security. Add it:

```ruby
lumina_filters :status, :user_id, :category_id
```

**Q: How does pagination work? Where are the page numbers?**

A: Pagination data comes in **response headers**, not the body:

```
X-Current-Page: 1
X-Last-Page: 5
X-Per-Page: 20
X-Total: 95
```

The response body is just the data array. Remember that pagination is disabled by default -- enable it with `lumina_pagination_enabled true`.

**Q: Can I search across related model fields?**

A: Yes. Use dot notation in `lumina_search`:

```ruby
lumina_search :title, :content, 'user.name'
```

Now `?search=john` searches in post title, post content, AND the user's name. Under the hood, Lumina uses `left_outer_joins` to traverse the relationship without excluding records that lack the association.

**Q: How do I get a count of related records without loading them?**

A: Use the `Count` suffix:

```bash
GET /api/posts?include=commentsCount
```

Response:
```json
{
    "id": 1,
    "title": "My Post",
    "comments_count": 15
}
```

Similarly, use `Exists` to check if any related records exist:
```bash
GET /api/posts?include=commentsExists
```

**Q: Can a user bypass permissions through includes?**

A: No. When using `?include=comments`, Lumina checks that the user has `comments.index` permission. If they do not, the request returns 403. This prevents permission bypass through eager loading.

**Q: What happens if `per_page` is set to a very large number?**

A: The `per_page` parameter is clamped between 1 and 100. If a client sends `?per_page=999`, it will be silently reduced to 100. If they send `?per_page=0`, it will be raised to 1.

---

## Real-World Examples

### Example 1: Blog Listing Page

```bash
# Published posts, newest first, with author info, only essential fields
GET /api/posts?filter[status]=published&sort=-published_at&include=user&fields[posts]=id,title,excerpt,published_at&fields[users]=id,name,avatar&per_page=10
```

Model configuration:

```ruby
class Post < Lumina::LuminaModel
  lumina_filters :status, :user_id
  lumina_sorts :created_at, :title, :published_at
  lumina_default_sort '-published_at'
  lumina_fields :id, :title, :excerpt, :status, :published_at
  lumina_includes :user, :comments, :tags
  lumina_search :title, :content

  lumina_pagination_enabled true
  lumina_per_page 10

  belongs_to :user
  has_many :comments
  has_many :tags
end
```

### Example 2: Admin Dashboard Search

```bash
# Search all posts for "refund", include comment counts, all statuses
GET /api/posts?search=refund&include=commentsCount&sort=-created_at&per_page=50
```

### Example 3: E-Commerce Product Listing

```ruby
class Product < Lumina::LuminaModel
  belongs_to :category
  belongs_to :brand
  has_many :reviews, dependent: :destroy
  has_many :images, dependent: :destroy

  validates :name, length: { maximum: 255 }, allow_nil: true
  validates :price, numericality: { greater_than_or_equal_to: 0 }, allow_nil: true

  lumina_filters :category_id, :brand_id, :in_stock
  lumina_sorts :price, :name, :created_at, :rating
  lumina_default_sort '-created_at'
  lumina_fields :id, :name, :price, :thumbnail, :rating
  lumina_includes :category, :brand, :reviews, :images
  lumina_search :name, :sku, :description

  lumina_pagination_enabled true
  lumina_per_page 24
end
```

```bash
# Products in category 5, sorted by price, with reviews, paginated
GET /api/products?filter[category_id]=5&sort=price&include=reviews&per_page=24
```
