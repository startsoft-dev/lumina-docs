...
Lumina includes an Artisan command that generates TypeScript interfaces from your registered models. It introspects the database schema, builds an OpenAPI 3.0 spec internally, and runs `openapi-typescript` to produce a `.d.ts` file.
...
This introspects all registered models from `config/lumina.php`, reads their database columns, and writes TypeScript interfaces to the configured output paths.
...
php artisan lumina:export-types --output=resources/js/types/lumina.d.ts
```
...
Or in `config/lumina.php`:

```php title="config/lumina.php"
return [
    'client_path' => env('LUMINA_CLIENT_PATH'),
    'mobile_path' => env('LUMINA_MOBILE_PATH'),
    // ...
];
```

When both `LUMINA_CLIENT_PATH` and `LUMINA_MOBILE_PATH` are set, the command writes to both:
- `{LUMINA_CLIENT_PATH}/src/types/lumina.d.ts`
- `{LUMINA_MOBILE_PATH}/src/types/lumina.d.ts`
...
2. Run `php artisan lumina:export-types`
